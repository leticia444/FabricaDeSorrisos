    ...
    private static async Task SeedBrinquedosAsync(AppDbContext context)
    {
        if (!context.Brinquedos.Any())
        {
            ...
            var produtos = new List<Brinquedo>
            {
                ...
                new Brinquedo { Nome = "Pelúcia Urso Rosa", Descricao = "Clássico para abraçar.", Preco = 89.90m, Estoque = 25, Ativo = true, MarcaId = estrela.Id, CategoriaId = catPelucias.Id, SubCategoriaId = subPeluciaTrad.Id, PersonagemId = semPers.Id, FaixaEtariaId = faixaBebe.Id, ImagemUrl = "/imagens/produtos/ursorosa.webp" },
